...
describe('<Posts />', () => {
  it('should render posts', () => {
    ...
    expect(screen.getAllByRole('heading', { name: /title example/i })).toHaveLength(3);
    // verificar o alt text da imagem
    expect(screen.getAllByRole('img', { name: /title example/i })).toHaveLength(3);
    // verificar os textos
    ...
    expect(screen.getByRole('img', { name: 'title example' })).toHaveAttribute('src', 'img/img.png');
  });
  // se não tiver props no post
  it("should not render posts if there's no props />", () => {
    render(<Posts />);

    expect(screen.queryByRole('heading', { name: /title example/ })).not.toBeInTheDocument();
  });
  // snapshot
